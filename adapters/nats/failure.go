...
// conservative built-in sanitizer to protect finalization bounds.
...
func handlerPanicText(handlerID string) string {
	if handlerID == "" {
		return "messenger/nats: handler panicked"
	}
	return fmt.Sprintf("messenger/nats: handler %s panicked", handlerID)
}
