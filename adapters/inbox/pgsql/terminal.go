...
func (b *backend) terminalSQL() terminalstate.SQL {
	return terminalstate.SQL{
		...
		Attempts: b.names.attempts, Generations: b.names.attemptGenerations, LockIdentity: b.statements.lockIdentity,
	}
}
...
func (b *backend) ConfirmTerminalHandoff(ctx context.Context, key inbox.Key, fingerprint inbox.Fingerprint) error {
	...
	if err != nil {
		return fmt.Errorf("inbox/pgsql: begin handoff confirmation: %w", err)
	}
	...
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("inbox/pgsql: commit handoff confirmation: %w", err)
	}
	return nil
}
...
func (b *backend) PruneTerminalAttempts(ctx context.Context, before time.Time, limit int) (int64, error) {
	tx, err := b.db.BeginTx(ctx, &sql.TxOptions{Isolation: sql.LevelReadCommitted})
	if err != nil {
		return 0, fmt.Errorf("inbox/pgsql: begin terminal prune: %w", err)
	}
	defer func() { _ = tx.Rollback() }()
	...
	if err := tx.Commit(); err != nil {
		return 0, fmt.Errorf("inbox/pgsql: commit terminal prune: %w", err)
	}
	return removed, nil
}
