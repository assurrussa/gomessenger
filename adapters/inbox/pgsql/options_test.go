...
func TestResolveNamespace(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantCompletedIndex: `"gomessenger_inbox_completed_at_idx"`,
		},
		{
			name:               "schema and prefix",
			options:            []Option{WithSchema("messaging"), WithTablePrefix("site_")},
			wantInbox:          `"messaging"."site_inbox"`,
			wantAttempts:       `"messaging"."site_inbox_attempts"`,
			wantGenerations:    `"messaging"."site_inbox_attempt_generations"`,
			wantCompletedIndex: `"site_inbox_completed_at_idx"`,
		},
		{
			name:               "empty prefix",
			...
			wantCompletedIndex: `"inbox_completed_at_idx"`,
		},
		{name: "invalid prefix", options: []Option{WithTablePrefix("site-")}, wantErr: true},
		{name: "invalid schema", options: []Option{WithSchema("tenant.data")}, wantErr: true},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if names.inbox != test.wantInbox || names.attempts != test.wantAttempts ||
				names.attemptGenerations != test.wantGenerations || names.completedAtIndex != test.wantCompletedIndex {
...
func TestCustomNamespaceRendersAllStatementsAndMigrations(t *testing.T) {
	...
	names, err := resolveNamespace(WithSchema("messaging"), WithTablePrefix("site_"))
	if err != nil {
	...
	for index := range statements.NumField() {
		...
		if !strings.Contains(statement, `"messaging"."site_inbox`) {
			t.Fatalf("statement %d is not schema-qualified: %s", index, statement)
		}
	}
	...
	for _, expected := range []string{
		`"messaging"."site_inbox"`,
		`"messaging"."site_inbox_attempts"`,
		`"messaging"."site_inbox_attempt_generations"`,
