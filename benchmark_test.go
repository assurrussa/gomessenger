...
func BenchmarkMessengerLocalEvent(b *testing.B) {
	event := messenger.MustEvent("benchmark.event", 1, messenger.JSON[int]())
	builder := messenger.NewBuilder(messenger.WithSource("urn:benchmark"))
	builder.Subscribe(event, "subscriber", func(context.Context, messenger.Message[int]) error { return nil })
	...
	b.ResetTimer()
	for range b.N {
		if _, err := m.Publish(context.Background(), event, 1); err != nil {
			b.Fatal(err)
		}
	}
}
